...
?>
	...
	if (($post_show_perm == 'show-bleft' || $post_show_perm == 'show-bright') && (($title == '' || !$title) || (!($title == '' || !$title) && $post_with_title_show_perm != 'hide'))) {
		...
		echo "<span class='permalink'>".suffusion_get_post_title_and_link($permalink_text)."</span>\n";
	}
...
<?php
	...
	if (!is_page()) {
		if ($post_meta_position == 'corners') {
			if (is_singular()) {
				if ('open' == $post->comment_status && ($post_show_comment == 'show-bleft' || $post_show_comment == 'show-bright')) {
?>
...
<?php
		...
		if (is_singular()) {
			if ('open' == $post->comment_status && ($post_show_comment == 'show' || $post_show_comment == 'show-tleft')) {
?>
			<span class="comments"><a href="#respond"><?php _e('Add comments', 'suffusion'); ?></a></span>
<?php
			}
...
<?php
			...
			$hide_title = get_post_meta($post->ID, 'suf_hide_page_title');
			if (!$hide_title) {
?>
...
function suffusion_include_jqfix_js() {
	...
	wp_enqueue_script('suffusion', get_template_directory_uri() . '/scripts/suffusion.js', array('jquery'), $version);
	if ($suffusion_is_ie6) {
		wp_enqueue_script('suffusion-ie6', get_template_directory_uri() . '/scripts/ie-fix.js', array('suffusion'), $version);
	}
...
function suffusion_in_place_post_navigation() {
	global $wp_query, $post;
	if (isset($_POST['post_id'])) {
		$post_id = esc_attr($_POST['post_id']);
		if (suffusion_admin_check_integer($post_id)) {
...
/**
 * Builds out the byline section for any layout using tiles (e.g. Tile Layout and Magazine Template)
 *
 * @param string $type
 * @return string
 */
function suffusion_get_tile_layout_bylines($type = 'mag_excerpt') {
	$enabled = 'suf_'.$type.'_bylines_enabled';
	$date = 'suf_'.$type.'_bylines_post_date';
	$permalinks = 'suf_'.$type.'_bylines_permalinks';
	$posted_by = 'suf_'.$type.'_bylines_posted_by';
	$comments = 'suf_'.$type.'_bylines_comments';
	$cats = 'suf_'.$type.'_bylines_categories';
	$tags = 'suf_'.$type.'_bylines_tags';

	global $post, $$enabled, $$date, $$permalinks, $$posted_by, $$comments, $$cats, $$tags;
	$bylines_enabled = $$enabled;
	if ($bylines_enabled == 'hide') {
		return '';
	}
	$show_post_date = $$date;
	...
	$ret = '';
	$ret .= "<div class='suf-tile-bylines fix'>\n";
	$ret .= "<ul class='suf-tile-byline-icons'>\n";
	...
	if ($show_posted_by != 'hide') {
		...
		$ret_trailer .= suffusion_print_author_byline(false);
		$ret_trailer .= "</li>\n";
	}
	...
	if ($show_tags != 'hide') {
		$tags = get_the_tags($post->ID);
		if ($tags != '') {
			$icon_a = "<a id='suf-tile-tags-{$post->ID}' class='suf-tile-tags-icon suf-tile-icon' href='#' title='".__('Tags', 'suffusion')."'><span>&nbsp;</span></a>";
			$ret .= "<li>$icon_a</li>\n";
			$ret_trailer .= "<li id='suf-tile-tags-text-{$post->ID}' class='suf-tile-tags-icon-text suf-tile-icon-text'><span class='icon'>&nbsp;</span>";
...
?>
		<div class="page-nav-left"><?php previous_posts_link('&nbsp;'); ?></div>
		<div class="page-nav-right"><?php next_posts_link('&nbsp;'); ?></div>
<?php
		}
    }
}
