...
class Suffusion_Options_Renderer {
	...
	function create_section_for_textarea($value) {
		...
		if (isset($rg_options[$value['id']]) && $rg_options[$value['id']] != "") {
			$text = stripslashes($rg_options[$value['id']]);
			$text = esc_attr($text);
			echo $text;
		}
		else {
	...
	function create_section_for_select($value) {
		...
		foreach ($value['options'] as $option_value => $option_text) {
			echo "<option ";
			if (isset($rg_options[$value['id']])) {
				selected($rg_options[$value['id']], $option_value);
			}
			else {
	...
	function create_section_for_multi_select($value) {
		...
		if (isset($rg_options[$value['id']])) {
			$consolidated_value = $rg_options[$value['id']];
		}
	...
	function create_section_for_radio($value) {
		...
		foreach ($value['options'] as $option_value => $option_text) {
			...
			if (isset($rg_options[$value['id']])) {
				$checked = checked(stripslashes($rg_options[$value['id']]), $option_value, false);
			}
			else {
	...
	function create_section_for_checkbox($value) {
		global $rg_options;
		$this->create_opening_tag($value);
		echo '<input type="checkbox" name="suffusion_options['.$value['id'].']" value="true" '.checked($rg_options[$value['id']], 'on')."/>\n";
		$this->create_closing_tag($value);
	}
	...
	function create_section_for_color_picker($value) {
		...
		if (!isset($rg_options[$value['id']])) {
			...
			$color_value = $rg_options[$value['id']];
		}
	...
	function create_section_for_sortable_list($value) {
		...
		if (!isset($rg_options[$value['id']])) {
			...
			$list_order = $rg_options[$value['id']];
		}
...
/*		foreach ($list_order_array as $list_item) {
	...
	function create_section_for_background($value) {
		...
		if (!isset($rg_options[$value['id']])) {
			...
			$default_txt = $rg_options[$value['id']];
			$default = $default_txt;
		...
		foreach ($value['options'] as $option_value => $option_text) {
			if (isset($rg_options[$value['id']])) {
				$checked = checked($rg_options[$value['id']], $option_value, false);
			}
			else {
				$checked = checked($value['std'], $option_value, false);
			}
	...
	function create_section_for_font($value) {
		global $rg_options, $suffusion_safe_font_faces;
		$this->create_opening_tag($value);
		$original = $value['std'];
		if (!isset($rg_options[$value['id']])) {
			$default = $value['std'];
...
//		echo "<input name=\"".$value['id']."\" type=\"submit\" value=\"".$value['std']."\" class=\"button\" onclick=\"submit_form(this, document.forms['form-$category'])\" />\n";
					...
					foreach ($this->nested_options as $inner_section => $inner_children) {
						if ($inner_section != $section) {
							foreach ($inner_children as $inner_child) {
								if (isset($rg_options[$inner_child])) {
									$options[$inner_child] = $rg_options[$inner_child];
								}
							}
						}
					}
	...
	function export_settings() {
		global $suffusion_inbuilt_options, $rg_options;
		$export = array();
		foreach ($suffusion_inbuilt_options as $value) {
			if ((isset($value['export']) && $value['export'] == 'ne') || !isset($value['id']) || $value['type'] == 'button') {
				continue;
			}
			if (!isset($rg_options[$value['id']]) && isset($value['std'])) {
				$export[$value['id']] = $value['std'];
			}
			else {
				$export[$value['id']] = $rg_options[$value['id']];
			}
		}
	...
	function render_custom_types($group) {
		...
		<div class='suf-options suf-options-$group suf-custom-type-settings' id='suf-options'>
			<div class='suf-options-page-header fix'>
				...
				?>
				<p style='clear: both; '>If you are moving from Suffusion to another theme you can use the post types generated here in the <a href='http://wordpress.org/extend/plugins/custom-post-type-ui/'>Custom Post Type UI</a> plugin</p>
