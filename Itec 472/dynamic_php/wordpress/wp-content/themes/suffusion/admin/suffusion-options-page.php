...
global $suffusion_options_file, $rg_options, $suffusion_unified_options, $suffusion_options_intro_page, $suffusion_options_theme_skinning_page, $suffusion_options_visual_effects_page, $suffusion_options_sidebars_and_widgets_page, $suffusion_options_blog_features_page, $suffusion_options_templates_page, $wp_version;
		...
		?>
			<div class="suf-header-nav">
				<div class="suf-header-nav-top fix">
					<h2 class='suf-header-1'>Suffusion &ndash; <?php echo $version; ?></h2>
...
function suffusion_admin_script_loader() {
	...
	if ($wp_version < 3.1) {
		...
		wp_enqueue_script('suffusion-jquery-ui-custom', get_template_directory_uri().'/admin/js/jquery-ui/jquery-ui-1.8.12.custom.js', array('jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-mouse', 'jquery-ui-position'));
	}
...
function suffusion_admin_style_loader() {
	...
	wp_enqueue_style('suffusion-admin-jq', get_template_directory_uri().'/admin/js/jquery-ui/css/jquery-ui-1.7.3.custom.css', array(), $local_version);
	wp_enqueue_style('suffusion-admin', get_template_directory_uri().'/admin/admin.css', array('suffusion-admin-jq'), $local_version);
}
...
function suffusion_version_checker() {
	...
	$feed_url = "";
	if (is_child_theme()) {
		$local_theme_data = get_theme_data(trailingslashit(STYLESHEETPATH).'style.css');
		if (isset($local_theme_data['Announcements Feed'])) {
			$feed_url = $local_theme_data['Announcements Feed'];
		}
	}
	if ($feed_url == "") {
		$local_theme_data = get_theme_data(trailingslashit(TEMPLATEPATH).'style.css');
		if (isset($local_theme_data['Announcements Feed'])) {
			$feed_url = $local_theme_data['Announcements Feed'];
		}
	}
