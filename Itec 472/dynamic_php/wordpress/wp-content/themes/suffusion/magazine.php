...
function suffusion_show_mag_single_excerpt() {
	...
	$ret .= suffusion_get_tile_layout_bylines('mag_excerpt');
		...
		if (isset($query->posts) && is_array($query->posts) && count($query->posts) > 0) {
			...
			while ($query->have_posts())  {
				...
				if ($suf_mag_catblocks_post_style == 'thumbnail') {
					...
					$image = suffusion_get_image(array('widget-thumb' => 'widget-' . $suf_mag_catblocks_thumbnail_size));
					$ret .= "<li class='fix'><div class='suf-widget-thumb'>" . $image . "</div><a href='" . get_permalink() . "' class='suf-widget-thumb-title'>" . get_the_title() . "</a>" . suffusion_excerpt(false, false, true, 'suffusion_excerpt_length_cat_block') . "</li>\n";
				}
				else {
