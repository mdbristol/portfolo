...
function suffusion_initialize_globals() {
	global $content_width, $suffusion_locale, $suffusion_reevaluate_styles, $suffusion_safe_font_faces, $rg_options, $suffusion_theme_name, $suffusion_theme_hierarchy;
	...
	$rg_options = get_option('suffusion_options');
...
function suffusion_include_files() {
	global $suffusion_unified_options, $rg_options, $suffusion_interactive_text_fields;
	...
	include_once (TEMPLATEPATH . '/widgets/suffusion-widgets.php');
	...
	if (isset($rg_options['suf_custom_php_file'])) {
		$custom_php_file = $rg_options['suf_custom_php_file'];
...
function suffusion_get_allowed_categories($prefix) {
	global $rg_options;
	$allowed = array();
	if (isset($rg_options[$prefix])) {
		$selected = $rg_options[$prefix];
...
function suffusion_setup_standard_actions_and_filters() {
	...
	add_action('wp_ajax_suffusion_in_place_post_navigation', 'suffusion_in_place_post_navigation');
	add_action('wp_ajax_nopriv_suffusion_in_place_post_navigation', 'suffusion_in_place_post_navigation');
...
function suffusion_create_additional_feeds() {
	...
	if (isset($rg_options['suf_custom_rss_feed_1']) && trim($rg_options['suf_custom_rss_feed_1']) != "") {
		echo "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".esc_attr($rg_options['suf_custom_rss_title_1'])."\" href=\"".$rg_options['suf_custom_rss_feed_1']."\" />\n";
	}
	if (isset($rg_options['suf_custom_rss_feed_2']) && trim($rg_options['suf_custom_rss_feed_2']) != "") {
		echo "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".esc_attr($rg_options['suf_custom_rss_title_2'])."\" href=\"".$rg_options['suf_custom_rss_feed_2']."\" />\n";
	}
	if (isset($rg_options['suf_custom_rss_feed_3']) && trim($rg_options['suf_custom_rss_feed_3']) != "") {
		echo "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".esc_attr($rg_options['suf_custom_rss_title_3'])."\" href=\"".$rg_options['suf_custom_rss_feed_3']."\" />\n";
	}
	if (isset($rg_options['suf_custom_atom_feed_1']) && trim($rg_options['suf_custom_atom_feed_1']) != "") {
		echo "<link rel=\"alternate\" type=\"application/atom+xml\" title=\"".esc_attr($rg_options['suf_custom_atom_title_1'])."\" href=\"".$rg_options['suf_custom_atom_feed_1']."\" />\n";
	}
	if (isset($rg_options['suf_custom_atom_feed_2']) && trim($rg_options['suf_custom_atom_feed_2']) != "") {
		echo "<link rel=\"alternate\" type=\"application/atom+xml\" title=\"".esc_attr($rg_options['suf_custom_atom_title_2'])."\" href=\"".$rg_options['suf_custom_atom_feed_2']."\" />\n";
	}
	if (isset($rg_options['suf_custom_atom_feed_3']) && trim($rg_options['suf_custom_atom_feed_3']) != "") {
		echo "<link rel=\"alternate\" type=\"application/atom+xml\" title=\"".esc_attr($rg_options['suf_custom_atom_title_3'])."\" href=\"".$rg_options['suf_custom_atom_feed_3']."\" />\n";
	}
...
function suffusion_get_home_link_html($position) {
	global $rg_options, $suffusion_interactive_text_fields;
	$retStr = "";
	$suf_show_home = "";
	$option_name = $position == 'top' ? 'suf_navt_show_home' : 'suf_show_home';
	if (!isset($rg_options[$option_name])) {
		$suf_show_home = "none";
	}
	else {
		$suf_show_home = $rg_options[$option_name];
	}
	...
	if ($suf_show_home == "text") {
		if ($rg_options[$option_name] === FALSE || $rg_options[$option_name] == null) {
			$suf_home_text = "Home";
		}
		else if (trim($rg_options[$option_name]) == "") {
			$suf_home_text = "Home";
		}
		else {
//			$suf_home_text = trim($suffusion_options[$option_name]);
			$suf_home_text = wpml_t('suffusion-interactive', $suffusion_interactive_text_fields[$option_name]."|$option_name", trim($rg_options[$option_name]));
		}
...
function suffusion_update_unified_options($option_name, $oldvalue, $newvalue) {
	global $suffusion_inbuilt_options, $rg_options, $skin_settings, $suffusion_theme_name, $suffusion_default_theme_name, $suffusion_unified_options;
	...
	foreach ($suffusion_inbuilt_options as $value) {
		if (isset($value['id']) && isset($value['type']) && $value['type'] != 'button') {
			...
			else if (isset($value['std']) && $rg_options[$value['id']] == $value['std']) {
				if (is_array($skin_settings) && isset($skin_settings[$value['id']])) {
					$unified_options_array[$value['id']] = $skin_settings[$value['id']];
				}
				else {
					$unified_options_array[$value['id']] = $rg_options[$value['id']];
				}
			}
			else {
				$unified_options_array[$value['id']] = $rg_options[$value['id']];
